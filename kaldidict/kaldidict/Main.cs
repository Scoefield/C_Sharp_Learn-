...
namespace kaldidict
{
    public partial class Main : Form
    {
        ...
        private void btnsearch_Click(object sender, EventArgs e)    //查找模块，在cmudict表查找
        {
        ...
        private void Dgvcmudata_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvcmudata.SelectedRows.Count > 0)
            {
                ...
                int count = Convert.ToInt32(MySqlHelper.ExecuteScalar(CommandType.Text, string.Format("SELECT count(*) FROM kaldidict WHERE dictword='{0}'", txtword.Text.Trim().Replace("'",@"\'"))));
                if (count > 0)
                {
        ...
        private void btnadd_Click(object sender, EventArgs e)   //添加单词字典模块，插入的是kaldidict表，更新compareword状态
        {
            ...
            if (count >0)
            {
                ...
                string sqlstr=string.Format("INSERT INTO kaldidict.kaldidict (dictword, lexicon, lexiconp, lexiconp_silprob) VALUES ('{0}','{1}','{2}','{3}')", txtword.Text.Trim().Replace("'", @"\'"), txtlexicon.Text.Trim().Replace("'", @"\'"), txtlexiconp.Text.Trim().Replace("'", @"\'"), txtlexiconp_silprob.Text.Trim().Replace("'", @"\'"));
        ...
        private void btnimportcompare_Click(object sender, EventArgs e)     //导入对比模块，与kaldidict表对比，没有的单词插入到compareword表中
        {
            ...
            if (ofd1.ShowDialog() == DialogResult.OK)
            {
                ...
                Application.DoEvents();
                var Kaldidictlist = data.Kaldidictlist;
                string alltxt = File.ReadAllText(ofd1.FileName);
                Regex regex1 = new Regex("[^a-zA-Z' ]");    //正则匹配
                ...
                alist.ForEach(a => {
                    if (!Kaldidictlist.Contains(a.Key.ToUpper())){  //利用键来对比
                        strlist.Add(a.Key);
                    }            
                });
                if (strlist.Count > 0)
        ...
        private void btnexportdict_Click(object sender, EventArgs e)    //生成最新dict模块，获取kaldi表数据
        {
            string currentpath = System.IO.Directory.GetCurrentDirectory(); //获取当前目录
            ...
            System.IO.Directory.CreateDirectory(dicdir);  //创建目录      
        ...
        private void btnreplacetext_Click(object sender, EventArgs e)   //生成替换文本
        {
        ...
        private void labdgvcmu_Click(object sender, EventArgs e)
        {
